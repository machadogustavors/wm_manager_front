...
import { createService, updateService } from '@src/services/servicesData';
import { ServiceBase, Service } from '@src/lib/types';
...
export function NewService({ onSuccess, onCancel, initialData, isEdit }: NewServiceProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit && initialData) {
        await updateService(initialData.id_servico, form as any);
      } else {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 shadow-xl w-full max-w-lg space-y-4">
        <h2 className="text-2xl font-bold" style={{ color: '#0092c4' }}>{isEdit ? 'Editar Serviço' : 'Novo Serviço'}</h2>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button type="submit" disabled={isLoading} className="px-6 py-2 rounded-lg font-bold shadow-md transition disabled:opacity-50" style={{ backgroundColor: '#0092c4', color: 'white' }}>
            {isLoading ? (isEdit ? 'Salvando...' : 'Salvando...') : (isEdit ? 'Salvar Alterações' : 'Salvar')}
          </button>
        </div>
      </form>
    </div>
